...
for (let i = 0; i < 3; i++)
{
  for (let j = 0; j < 3; j++)
  {
    for (let k = 0; k < 3; k++)
    {
      translates.push([w*(k-1), -w*(j-1), w*(1-i)]);
    }
  }
}
...
function init(colorData) {
  ...
  for (let i = 0; i < dims[0]; i++) // layer
  {
    geometries.push([]);
    materials.push([]);
    meshes.push([]);
    for (let k = 0; k < dims[1]; k++) // which cube in the layer
    {
  ...
  rotateIdxs = {
    ...
    "L": [[0,0],[0,3],[0,6],[1,6],[2,6],[2,3],[2,0],[1,0]],
    "U": [[0,0],[1,0],[2,0],[2,1],[2,2],[1,2],[0,2],[0,1]],
    "D": [[0,6],[0,7],[0,8],[1,8],[2,8],[2,7],[2,6],[1,6]],
    "F": [[0,0],[0,1],[0,2],[0,5],[0,8],[0,7],[0,6],[0,3]],
    "B": [[2,0],[2,1],[2,2],[2,5],[2,8],[2,7],[2,6],[2,3]]
  }
...
function rotation(type, ct, callback, params) {
  ...
  if (ct > maxCt) {
    ...
    for (let i = 0; i < n; i++)
    {
      meshInds[idxs[(i+2)%n][0]][idxs[(i+2)%n][1]] = tmpIdxs[i];
    }
    if (callback) {
      callback(params);
    }
